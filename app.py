...
def vehicle_damage_assessment():
    ...
    if uploaded_file is not None:
        ...
        if st.button('Analyze Image'):
            ...
            # Call your predict_damage_severity function
            severity_label = predict_damage_severity(image_np)
            
            # Display the predicted severity label
            ...
            # Call your detect_damage function
            detection_result, detection_labels = detect_damage(image_np)
            
            # Assuming sv.BoxAnnotator().annotate returns an annotated image
            annotated_image = sv.BoxAnnotator().annotate(image_np, detection_result, labels=detection_labels)
            
            # Convert the annotated image back to PIL format
            annotated_image_pil = Image.fromarray(annotated_image)
            
            # Display the annotated image
            st.image(annotated_image_pil, caption="Damage Detection Result", use_column_width=True)
